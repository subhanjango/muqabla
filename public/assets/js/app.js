...
var getDataForDataTable = (URL , dataColumns , options = []) => {
  ...
  return new Promise(function(resolve , reject){
    ...
    for(let key in options) {
      switch (options[key]['action']) {
        
        case 'edit' :
        html += '<button title="edit" class="btn btn-primary" onClick = "'+options[key]['callingFunc']+'"><i class="ti ti-pencil-alt2"></i></button>'; 
        break;

        case 'view' :
        html += '<button title="view" class="btn btn-success" onClick = "'+options[key]['callingFunc']+'"><i class="ti ti-eye"></i></button>'; 
        break;

        case 'delete' :
        html += '<button title="delete" class="btn btn-danger" onClick = "'+options[key]['callingFunc']+'"><i class="ti ti-trash"></i></button>'; 
        break;
      ...
      .then(function(response){
          ...
          for(let key1 in response) {
            ...
            let dataSetRow = [];
              ...
              for(let key2 in dataColumns) {


               let value = response[key1][dataColumns[key2]] ? response[key1][dataColumns[key2]] : '';
               
               if(dataColumns[key2] == 'created_at') {
               ...
               if(k === dataColumns.length) { 
                 dataSetRow.push(html);
...
$(function(){
  ...
  let current_module = getCurrentPageName();

  $('#'+current_module).addClass('active');
});
